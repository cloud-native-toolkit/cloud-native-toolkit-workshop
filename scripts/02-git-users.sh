...
for (( c=1; c<=COUNT_USERS; c++ )); do
  ...
  curl -X POST -H "Authorization: token ${ACCESS_TOKEN}" -H "Content-Type: application/json" -d "{ \"username\": \"user${c}\", \"password\": \"user${c}\", \"email\": \"user${c}@cloudnativetoolkit.dev\" }" "${GIT_URL}/api/v1/admin/users"
done
